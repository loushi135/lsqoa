MailPanelView = Ext.extend(Ext.ux.Portlet,{
	tools : null,
	constructor : function(_cfg){
		Ext.applyIf(this,_cfg);
		this.initTool();
		MailPanelView.superclass.constructor.call(this,{
			id : "MailPanelView",
			title : "我的邮件",	
			iconCls : "menu-mail",
			tools : this.tools,
//			autoLoad : {
//				url : __ctxPath	+ "/mail/mboxMailUtil.do",
//				scripts : true
//			}
		})
	},
		...
		this.tools = [
			{
				id : "refresh",
				handler : function() {
					Ext.getCmp("MailPanelView").getUpdater().update(
						__ctxPath + "/mail/mboxMailUtil.do");
				}
			}
		]
	}
...
ArticleSendPanelView = Ext.extend(Ext.ux.Portlet, {
	tools : null,
	constructor : function(a) {
		Ext.applyIf(this, a);
		this.initTool();
		ArticleSendPanelView.superclass.constructor.call(this, {
			id : "ArticleSendPanelView",
			title : "最新发文",
			iconCls : "menu-notice",
			tools : this.tools,
			items:[
					...
					items:[
						{
							...
							id:'goldM',
							autoLoad : {
								url : __ctxPath + "/info/displayArticleSend.do?typeId=1"
							}
						},{
							title : "朗捷通发文",
							iconCls : "menu-notice",
							id:'ljt',
							autoLoad : {
								url : __ctxPath + "/info/displayArticleSend.do?typeId=2"
							}
						}
					],
					listeners :{
//						tabchange: function(tabPanel,activePanel){
//								if(activePanel.getId()=='goldM'){
//									activePanel.getUpdater().update(__ctxPath + "/info/displayArticleSend.do?typeId=1");
//								} else if(activePanel.getId()=='ljt'){
//									activePanel.getUpdater().update(__ctxPath + "/info/displayArticleSend.do?typeId=2");
//								}
//						},
...
//							obj.setActiveTab(0);
	...
	initTool : function() {
		this.tools = [{
					id : "refresh",
					handler : function() {
						Ext.getCmp("goldM").getUpdater().update(__ctxPath + "/info/displayArticleSend.do?typeId=1");
						Ext.getCmp("ljt").getUpdater().update(__ctxPath + "/info/displayArticleSend.do?typeId=2");
					}
				},
				 {
					id : "close",
...
PhotoView = Ext.extend(Ext.ux.Portlet, {
	tools : null,
	constructor : function(a) {
		Ext.applyIf(this, a);
		this.initTool();
		PhotoView.superclass.constructor.call(this, {
			id : "PhotoView",
			title : "公司相册",
			iconCls : "menu-news",
			tools : this.tools,
...
//			height : 405,
			autoLoad : {
				url:__ctxPath+"/hrm/imageViewPhoto.do",
				scripts : true
			}
		});
	},
	initTool : function() {
		this.tools = [ {
			id : "refresh",
			handler : function() {
				Ext.getCmp("PhotoView").getUpdater().update({
				url:__ctxPath+"/hrm/imageViewPhoto.do",
					scripts : true
				});
			}
...
PublicDocumentPanelView = Ext.extend(Ext.ux.Portlet, {
	tools : null,
	constructor : function(a) {
		Ext.applyIf(this, a);
		this.initTool();
		BbsView.superclass.constructor.call(this, {
			id : "PublicDocumentPanelView",
			title : "公共文档",
			iconCls : "menu-news",
			tools : this.tools,
			border : false,
			autoLoad : {
				url : __ctxPath+ "/document/displayPublicDocument.do?start=0&limit=5",
				scripts : true
			}
		});
	},
	initTool : function() {
		this.tools = [ {
			id : "refresh",
			handler : function() {
				Ext.getCmp("PublicDocumentPanelView").getUpdater().update({
			    url : __ctxPath+ "/document/displayPublicDocument.do?start=0&limit=5",
					scripts : true
				});
			}
...
BbsView = Ext.extend(Ext.ux.Portlet, {
	tools : null,
	constructor : function(a) {
		Ext.applyIf(this, a);
		this.initTool();
		BbsView.superclass.constructor.call(this, {
			...
			border : false,
			autoLoad : {
				url : __ctxPath + "/bbs/indexViewBbsTopic.do?start=0&limit=5",
				scripts : true
			}
		});
	},
	initTool : function() {
		this.tools = [ {
			id : "refresh",
			handler : function() {
				Ext.getCmp("BbsView").getUpdater().update({
				url : __ctxPath + "/bbs/indexViewBbsTopic.do?start=0&limit=5",
					scripts : true
				});
			}
...
//				},
				{
					...
					boxLabel : "公司相册",
					hideLabel : true,
					id : "PhotoViewCheckBox",
					name : "PhotoView"
				},{
					xtype : "checkbox",
					boxLabel : "公共文档",
					hideLabel : true,
					id : "PublicDocumentPanelViewCheckBox",
					name : "PublicDocumentPanelView"
				}]
			}, {
				layout : "form",
				columnWidth : 0.5,
...
//				   }, 
				...
				,{
					...
					boxLabel : "最新发文",
					hideLabel : true,
					id : "ArticleSendPanelViewCheckBox",
					name : "ArticleSendPanelView"
				}
//				, {
//					xtype : "checkbox",
//					boxLabel : "我的邮件",
		...
		this.buttons = [
				{
					...
					handler : function() {
						...
						var array = [ "DiaryPanelView", "TaskPanelView",
								...
								"MyMailPanelView","PhotoView","BbsView","PublicDocumentPanelView","ArticleSendPanelView"];
...
AppHome = Ext.extend(Ext.Panel, {
	...
	initUIComponents : function() {
		...
		if (confs == null || confs == undefined || confs.length < 1) {
			...
			var p5 = {
				panelId : 'MailPanelView',
				column : 1,
				row : 0
			}
			var p6 = {
				panelId : 'ArticleSendPanelView',
				column : 1,
				row : 1
			}
			confs.push(p5);
