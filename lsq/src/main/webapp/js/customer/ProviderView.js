	...
	initUIComponents : function() {
		this.searchPanel = new Ext.FormPanel({
			...
			items : [{
						...
						iconCls : "search",
						handler : function() {
							var c = Ext.getCmp("ProviderSearchForm");
							var d = Ext.getCmp("ProviderGrid");
		...
		var a = new Ext.grid.ColumnModel({
			columns : [b, new Ext.grid.RowNumberer(), {
						...
						renderer : function(c) {
							if (c == "1") {
								return '<img title="一级供应商" src="'
										+ __ctxPath
										+ '/images/flag/customer/grade_one.png"/>';
							} else {
								if (c == "2") {
									return '<img title="二级供应商" src="'
											+ __ctxPath
						...
						renderer : function(f, e, c, i, d) {
							var h = c.data.providerId;
							var g = "";
							if (isGranted("_ProviderDel")) {
								g = '<button title="删除" value=" " class="btn-del" onclick="ProviderView.remove('
										+ h + ')">&nbsp;&nbsp;</button>';
							}
							if (isGranted("_ProviderEdit")) {
								g += '&nbsp;<button title="编辑" value=" " class="btn-edit" onclick="ProviderView.edit('
										+ h + ')">&nbsp;&nbsp;</button>';
							}
							return g;
						}
			...
			this.topbar.add(new Ext.Button({
						...
						handler : function() {
							...
							for (var d = 0; d < c.length; d++) {
								f.push(c[d].data.providerId);
							}
							ProviderView.remove(f);
						}
					}));
		}
		this.topbar.add(new Ext.Button({
		...
		this.gridPanel.addListener("rowdblclick", function(d, c, f) {
					d.getSelectionModel().each(function(e) {
								if (isGranted("_ProviderEdit")) {
									ProviderView.edit(e.data.providerId);
								}
							});
				});
	}
});
ProviderView.remove = function(b) {
	var a = Ext.getCmp("ProviderGrid");
	Ext.Msg.confirm("信息确认",
			'删除供应商，则该供应商下的<font color="red">产品</font>也删除，您确认要删除该记录吗？', 150,
			function(c) {
				if (c == "yes") {
					Ext.Ajax.request({
								url : __ctxPath
