	...
	initUIComponents : function() {
		this.searchPanel = new Ext.FormPanel({
			...
			items : [{
						...
						iconCls : "search",
						handler : function() {
							var c = Ext.getCmp("NoticeSearchForm");
							var d = Ext.getCmp("NoticeGrid");
			...
			columns : [b, new Ext.grid.RowNumberer(), {
						...
						renderer : function(f, e, c, i, d) {
							...
							if (isGranted("_NoticeDel")) {
								g = '<button title="删除" value=" " class="btn-del" onclick="NoticeView.remove(' + h + ')"></button>';
							}
							if (isGranted("_NoticeEdit")) {
								g += '&nbsp;<button title="编辑" value=" " class="btn-edit" onclick="NoticeView.edit(' + h + ')"></button>';
							}
							return g;
						}
					}],
		...
		this.gridPanel.addListener("rowdblclick", function(d, c, f) {
			d.getSelectionModel().each(function(e) {
				if (isGranted("_NoticeEdit")) {
					NoticeView.edit(e.data.noticeId);
				}
			});
		});
	}
});
NoticeView.remove = function(b) {
	var a = Ext.getCmp("NoticeGrid");
