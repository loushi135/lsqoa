					...
					, {
						...
						renderer : function(g, f, d, j, e) {
							...
							if (isGranted("_EmpProfileDel")) {
								h += '<button title="删除" value=" " class="btn-del" onclick="EmpProfileView.remove('
										+ i + ')">&nbsp;&nbsp;</button>';
							}
							if (isGranted("_EmpProfileEdit") && c != 1) {
								h += '<button title="编辑" value=" " class="btn-edit" onclick="EmpProfileView.edit('
										+ i + ')">&nbsp;&nbsp;</button>';
							}
							if (c != 1 && c != 2) {
								if (isGranted("_EmpProfileCheck")) {
									h += '<button title="审核" value=" " class="btn-empProfile-check" onclick="EmpProfileView.check('
											+ i + ')">&nbsp;&nbsp;</button>';
								}
							} else {
								if (isGranted("_EmpProfileQuery")) {
									h += '<button title="查看" value=" " class="btn-readdocument" onclick="EmpProfileView.look('
											+ i + ')">&nbsp;&nbsp;</button>';
								}
							}
							if (isGranted("_EmpProfileQuery")) {
								h += '<button title="查看操作记录" value=" " class="btn-operation" onclick="EmpProfileView.operation('
										+ i + ')">&nbsp;&nbsp;</button>';
							}
							return h;
						}
