							...
							renderer : function(n, m, j, l, o) {
								...
								if (i != null) {
									...
									if (d) {
										...
										if (isGranted("_FlowSetting")) {
											k += '&nbsp;<button title="流程设置通知用户" class="btn-setting" onclick="ProDefinitionView.settingUser('
													+ ""
													+ h
													+ ",'"
													+ g
...
ProDefinitionView.settingUser = function(defId, name) {
	new ProDefNoticeForm(defId,name).getView().show();
};
