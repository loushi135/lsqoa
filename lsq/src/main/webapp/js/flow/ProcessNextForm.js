ProcessNextForm = Ext.extend(Ext.Panel, {
	...
	initComponent : function() {
		...
		var detailPanel = new Ext.Panel({
					...
					autoScroll : true,
					autoLoad : {
						url : __ctxPath + "/flow/processRunDetail.do?randId="
					...
					if (isFormPanel) {
						Ext.Ajax.request({
							...
							success : function(response, options) {
											...
											.insert(
													...
													self
															.genFormButton(
																	...
																	activityName,
																	object.data[i].destinationDisplay,
																	object.executionId));
								}
								...
								var backReturnAssagebtn= new Ext.Button({
									text : "转至[审批人]",
									iconCls : "btn-transitionback",
									id:'backReturnBtn',
									handler : function() {
										if (destName == "") {
											freeTransCombo.focus();
											Ext.ux.Toast.msg("操作信息", "请选择跳转的任务!");
											return;
										}
										var form = null;
										if (!isFormPanel) {
								...
								self.getTopToolbar().add(backReturnAssagebtn);
								self.getTopToolbar().add(freeTransCombo);
								self.getTopToolbar().add(new Ext.Toolbar.Separator());
	...
	genFormButton : function(b, d, a, e, f, destinationDisplay, executionId) {
		var c = b;

		var btnId = '';
		var iconCls='';
		if (destinationDisplay == '审批不通过') {
			btnId = 'unPass';
			iconCls = "btn-transitionback";
		} else {
			btnId = 'pass';
					...
					if (g.isValid()) {
						g.submit({
							...
							params : {
								...
								activityName : f,
								destName : e,
								returnBack : destinationDisplay=='审批不通过'?1:-1,
								piId:executionId
							},
