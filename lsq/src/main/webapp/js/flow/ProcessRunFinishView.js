...
ProcessRunFinishView.prototype.grid = function() {
    ...
    var a = new Ext.grid.ColumnModel({
        ...
        },
        {
            ...
            renderer: function(l, j, f, i, n) {
                ...
                var g = '&nbsp;<button type="button" title="审批明细" value=" " class="btn-flowView" onclick="ProcessRunFinishView.detail(' + k + "," + e + ",'" + m + "','" + h + "')\"></button>";
                return g;
            }
        }],
        defaults: {
    ...
    c.addListener("rowdblclick",
    function(g, f, h) {
        g.getSelectionModel().each(function(e) {
            ProcessRunFinishView.detail(e.data.runId, e.data.defId, e.data.piId, e.data.subject);
        });
    });
    return c;
};
