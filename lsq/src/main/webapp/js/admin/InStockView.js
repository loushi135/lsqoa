			...
			columns : [b, new Ext.grid.RowNumberer(), {
						...
						renderer : function(f, e, c, i, d) {
							...
							if (isGranted("_InStockDel")) {
								g = '<button title="删除" value=" " class="btn-del" onclick="InStockView.remove('
										+ h + ')">&nbsp;</button>';
							}
							if (isGranted("_InStockEdit")) {
								g += '&nbsp;<button title="编辑" value=" " class="btn-edit" onclick="InStockView.edit('
										+ h + ')">&nbsp;</button>';
							}
							return g;
						}
		...
		if (isGranted("_InStockAdd")) {
			this.topbar.add(new Ext.Button({
						...
						handler : function() {
							...
							Ext.getCmp("InStockFormContainer")
									.remove("stockNo");
