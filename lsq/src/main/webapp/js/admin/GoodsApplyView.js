		...
		this.store = new Ext.data.Store({
					...
					reader : new Ext.data.JsonReader({
								...
								fields : [{
										...
										"proposer", "notes", "approvalStatus","processRunId"]
							}),
					remoteSort : true
				});
		...
		var a = new Ext.grid.ColumnModel({
			columns : [b, new Ext.grid.RowNumberer(), {
						...
						renderer : function(g, f, d, j, e) {
							...
							var processRunId=d.data.processRunId;
							var h = "";
							if (isGranted("_GoodsApplyDel")) {
								h = '<button title="删除" value=" " class="btn-del" onclick="GoodsApplyView.remove('
										+ i + ')">&nbsp;</button>';
							}
							if (isGranted("_GoodsApplyEdit")) {
								if (c == 0) {
									h += '&nbsp;<button title="编辑" value=" " class="btn-edit" onclick="GoodsApplyView.edit('
											+ i + ')">&nbsp;</button>';
								}
							}
							
							h += '<button title="查看审批表单" value=" " class="btn-flowView" onclick="AppUtil.selectProcessForm('+ processRunId +')"></button>';
							return h;
						}
					}],
			defaults : {
