OfficeGoodsForm = Ext.extend(Ext.Window, {
	...
	initUIComponents : function() {
		var a = __ctxPath + "/admin/treeOfficeGoodsType.do?method=1";
		var b = new TreeSelector("goodsTypeSelect", a, "所属分类", "typeId");
		...
		this.buttons = [{
			...
			handler : function() {
				...
				if (Ext.getCmp("stockCounts") != null) {
					var f = Ext.getCmp("stockCounts").getValue();
					var d = Ext.getCmp("counts").getValue();
				}
				var e = Ext.getCmp("goodsTypeSelect").getValue();
				if (e != null && e != "" && e != "undefined") {
					if (f != d) {
						Ext.Msg.confirm("操作信息", "你已经修改了库存，是否继续保存?",
								function(g) {
									if (g == "yes") {
