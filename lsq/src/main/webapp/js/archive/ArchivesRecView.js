					...
					initComponents : function() {
												...
												renderer : function(h, f, d, j,
													...
													if (g == 0) {
														if (isGranted("_ArchivesRecDel")) {
															i += '<button title="删除" value=" " class="btn-del" onclick="ArchivesRecView.remove('
																	+ c
																	+ ')"></button>';
														}
														if (isGranted("_ArchivesRecEdit")) {
															i += '<button title="编辑" value=" " class="btn-edit" onclick="ArchivesRecView.edit('
																	+ c
																	+ ')"></button>';
														}
													} else {
														if (isGranted("_ArchivesRecQuery")) {
															i += '<button title="查看" value=" " class="btn-readdocument" onclick="ArchivesRecView.detail('
																	+ c
																	+ ')"></button>';
														}
													}
						...
						this.gridPanel.addListener("rowdblclick", function(d,
							...
							d.getSelectionModel().each(function(e) {
								if (e.data.status == 0) {
									ArchivesRecView.edit(e.data.archivesId);
								}
							});
						});
					},
