				...
				items : [{
							...
							listeners : {
								select : function(h, d, f) {
									var e = Ext.getCmp("profile.photo");
	...
	a.getForm().load({
		...
		success : function(f, g) {
			...
			if (d.value != "" && d.value != null && d.value != "undefined") {
				h.body.update('<img src="' + __ctxPath + "/attachFiles/"
						+ d.value + '" width="100%" height="100%"/>');
			} else {
				if (e.value == "0") {
...
ProfileForm.uploadPhotoBtn = function(c) {
	...
	if (a.value != "" && a.value != null && a.value != "undefined") {
		var d = "再次上传需要先删除原有图片,";
		Ext.Msg.confirm("信息确认", d + "是否删除？", function(e) {
			if (e == "yes") {
				Ext.Ajax.request({
					...
					success : function() {
						if (c != "" && c != null && c != "undefined") {
							Ext.Ajax.request({
								...
								success : function() {
									a.setValue("");
									var h = Ext.getCmp("profileTitle");
									var i = Ext.getCmp("displayProfilePhoto");
									if (h.value == 1) {
										i.body
												.update('<img src="'
														+ __ctxPath
							...
							a.setValue("");
							var g = Ext.getCmp("profileTitle");
							var f = Ext.getCmp("displayProfilePhoto");
							if (g.value == 1) {
								f.body.update('<img src="' + __ctxPath
										+ '/images/default_image_male.jpg" />');
							} else {
								f.body
										.update('<img src="'
...
ProfileForm.deleteUserPhoto = function(b) {
	...
	if (a.value != null && a.value != "" && a.value != "undefined") {
		var c = "照片一旦删除将不可恢复,";
		Ext.Msg.confirm("确认信息", c + "是否删除?", function(d) {
			if (d == "yes") {
				Ext.Ajax.request({
					url : __ctxPath + "/system/deleteFileAttach.do",
					method : "post",
						...
						if (b != "" && b != null && b != "undefined") {
							Ext.Ajax.request({
								...
								success : function() {
									a.setValue("");
									var h = Ext.getCmp("profileTitle");
									var g = Ext.getCmp("displayProfilePhoto");
									if (h.value == 1) {
										g.body
												.update('<img src="'
														+ __ctxPath
							...
							a.setValue("");
							var f = Ext.getCmp("profileTitle");
							var e = Ext.getCmp("displayProfilePhoto");
							if (f.value == 1) {
