	...
	initUIComponents : function() {
		this.searchPanel = new Ext.FormPanel({
			...
			items : [{
					...
					iconCls : "search",
					handler : function() {
						var c = Ext.getCmp("AppRoleSearchForm");
		...
		var a = new Ext.grid.ColumnModel({
			columns : [b, new Ext.grid.RowNumberer(), {
					...
					renderer : function(c) {
						var d = "";
						if (c == "1") {
							d += '<img title="激活" src="' + __ctxPath + '/images/flag/customer/effective.png"/>';
						} else {
							d += '<img title="禁用" src="' + __ctxPath + '/images/flag/customer/invalid.png"/>';
						}
						return d;
					}
				}, {
						...
						if (c != -1) {
							if (h == "0") {
								if (isGranted("_AppRoleDel")) {
									f = '<button title="删除" value=" " class="btn-del" onclick="AppRoleView.remove(' + c + ')"></button>';
								}
								if (isGranted("_AppRoleEdit")) {
									f += '&nbsp;<button title="编辑" value=" " class="btn-edit" onclick="AppRoleView.edit(' + c + ')"></button>';
								}
								if (isGranted("_AppRoleGrant")) {
									f += '&nbsp;<button title="授权" value=" " class="btn-grant" onclick="AppRoleView.grant(' + c + ",'" + d + "')\">&nbsp;</button>";
								}
							} else {
								f = '<button title="复制" value=" " class="btn-copyrole" onclick="AppRoleView.copy(' + c + ')"></button>';
							}
						}
						return f;
					}
		...
		this.gridPanel.addListener("rowdblclick", function(d, c, f) {
			d.getSelectionModel().each(function(e) {
				if (e.data.isDefaultIn == "0" && e.data.roleId != -1) {
		...
		a.add(new Ext.Button({
			...
			handler : function() {
				...
				for (var c = 0; c < b.length; c++) {
					if (b[c].data.isDefaultIn == "0" && b[c].data.roleId != -1) {
