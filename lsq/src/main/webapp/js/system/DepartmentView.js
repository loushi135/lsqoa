...
DepartmentView.prototype.setup = function() {
	...
	var l = new Ext.grid.ColumnModel({
		columns : [d, new Ext.grid.RowNumberer(), {
					...
					renderer : function(n) {
						var o = "";
						if (n == "1"||n == "3") {
							o += '<img title="激活" src="' + __ctxPath
									+ '/images/flag/customer/effective.png"/>'
						} else {
							o += '<img title="禁用" src="' + __ctxPath
	...
	function g(o, n, p) {
		o.getSelectionModel().each(function(r) {
					var q = r.data.userId;
					if (isGranted("_AppUserEdit") && q != 1) {
						AppUserView.edit(q, r.data.username)
					}
				})
	}
	k.load({
