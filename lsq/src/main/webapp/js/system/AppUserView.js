AppUserView = Ext.extend(Ext.Panel, {
			...
			onSearch : function(c) {
				...
				if (a.getForm().isValid()) {
					a.getForm().submit({
						waitMsg : "正在提交查询",
						url : __ctxPath + "/system/listHasEMPAppUser.do",
...
AppUserView.prototype.initSearchPanel = function() {
	this.searchPanel = new Ext.FormPanel({
				...
				items : [{
							...
							xtype : "datefield",
							format : "Y-m-d",
							name : "Q_u.accessionTime_D_GT"
						}, {
							text : "至"
						}, {
							xtype : "datefield",
							format : "Y-m-d",
							name : "Q_u.accessionTime_D_LT"
						}, {
							xtype : "button",
							text : "查询",
							iconCls : "search",
	...
	var a = new Ext.grid.ColumnModel({
		columns : [e, new Ext.grid.RowNumberer(), {
					...
					renderer : function(f) {
						var g = "";
						if (f == "1" || f == '3') {
							g += '<img title="激活" src="' + __ctxPath
									+ '/images/flag/customer/effective.png"/>';
						} else {
							g += '<img title="禁用" src="' + __ctxPath
					...
					renderer : function(j, i, g, m, h) {
						...
						// if (l != 1) {
						if (isGranted("_AppUserDel")) {
							k += '<button title="删除" value=" " class="btn-del" onclick="AppUserView.remove('
									+ l + ')"></button>';
						}
