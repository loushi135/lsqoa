	...
	var a = new Ext.grid.ColumnModel({
		columns : [e, new Ext.grid.RowNumberer(), {
					...
					renderer : function(k, h, f) {
						if (k == "" || k == "0") {
							...
							for (var g = 0; g < j.length; g++) {
								l += '<a href="#" onclick="FileAttachDetail.show(' + j[g].fileId + ');" class="attachment">' + j[g].fileName + "</a>";
								l += "&nbsp;";
							}
							return l;
						}
					...
					renderer : function(i, h, f, l, g) {
						var k = f.data.docId;
						var j = '<button title="查看" value="" class="btn-readdocument" onclick="PublicDocumentView.detail(' + k + ')">&nbsp;</button>';
						if (isGranted("__ALL")) {
							j += '<button title="删除" value="" class="btn-del" onclick="PublicDocumentView.remove(' + k + ')">&nbsp;</button>';
						}
						return j;
					}
				}],
	...
	var a = new Ext.data.Store({
		...
		reader : new Ext.data.JsonReader({
			...
			fields : [{
					...
					}, "fullname", "docName", "content", "createtime", "haveAttach", "attachFiles", "isShared"]
		}),
		remoteSort : true
	});
	return a;
							...
							if (k == "yes") {
								Ext.Ajax.request({
									...
									success : function() {
										...
										Ext.ux.Toast.msg("信息提示", "成功删除所选记录！");
										var l = Ext.getCmp("PublicDocumentGrid");
										l.getStore().reload({
											params : {
												start : 0,
												limit : 25
