...
public class FormData extends BaseModel {
	public static final Short SHOWED = Short.valueOf((short) 1);

	public static final Short UNSHOWED = Short.valueOf((short) 0);
	...
	protected Short isShowed = Short.valueOf((short) 1);
	...
	public String getVal() {
		...
		if ("date".equals(this.fieldType)) {
			...
			return this.dateValue == null ? null : sdf
		...
		if ("int".equals(this.fieldType)) {
			return this.intValue == null ? null : this.intValue
					.toString();
		}

		if ("long".equals(this.fieldType)) {
			return this.longValue == null ? null : this.longValue
					.toString();
		}

		if ("decimal".equals(this.fieldType)) {
			return this.decValue == null ? null : this.decValue
					.toString();
		}
	...
	public void setValue(String val, String type) {
		...
		try {
			if ("varchar".equals(type))
				...
				this.boolValue = Short
						.valueOf((short) ("1".equals(val) ? 1 : 0));
			else if (("date".equals(type))
					|| ("datetime".equals(type)))
				this.dateValue = DateUtils.parseDate(val,
						new String[] { "yyyy-MM-dd HH:mm:ss","yyyy-MM-dd HH:mm", "yyyy-MM-dd","yyyy-MM","yyyy" });
	...
	public Long getFormId() {
		return getProcessForm() == null ? null : getProcessForm()
				.getFormId();
	}
