...
public class SalaryPayoffAction extends BaseAction {
	...
	public String get() {
		SalaryPayoff salaryPayoff = (SalaryPayoff) this.salaryPayoffService
				.get(this.recordId);
	...
	public String save() {
		if (this.salaryPayoff.getRecordId() == null) {
			this.salaryPayoff.setCheckStatus(Short
					.valueOf(SalaryPayoff.CHECK_FLAG_NONE));
			this.salaryPayoff.setRegTime(new Date());
			this.salaryPayoff.setRegister(ContextUtil.getCurrentUser()
					.getFullname());
		}
	...
	public String check() {
		SalaryPayoff checkSalaryPayoff = (SalaryPayoff) this.salaryPayoffService
		...
		checkSalaryPayoff.setCheckName(ContextUtil.getCurrentUser()
				.getFullname());
		checkSalaryPayoff.setCheckStatus(this.salaryPayoff
				.getCheckStatus());
		checkSalaryPayoff.setCheckOpinion(this.salaryPayoff
				.getCheckOpinion());
