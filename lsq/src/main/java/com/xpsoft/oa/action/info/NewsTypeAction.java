...
public class NewsTypeAction extends BaseAction {
	...
	public String tree() {
		...
		for (NewsType newsType : list) {
			sb.append("{id:'").append(newsType.getTypeId())
		...
		if (this.newsType != null) {
			...
			if ((top != null) && (top.shortValue() > sn.shortValue())) {
				for (int i = 0; i < top.shortValue() - sn.shortValue(); i++) {
					setNewsType(this.newsTypeService.findBySn(Short
							.valueOf((short) (sn.shortValue() + i + 1))));
					this.newsType.setSn(Short.valueOf((short) (sn
							.shortValue() + i)));
					this.newsTypeService.save(this.newsType);
				}
			}
	...
	public String sort() {
		Integer opt = Integer.valueOf(Integer.parseInt(getRequest()
				.getParameter("opt")));
		Long typeId = Long
				.valueOf(getRequest().getParameter("typeId"));
		setNewsType((NewsType) this.newsTypeService.get(typeId));
