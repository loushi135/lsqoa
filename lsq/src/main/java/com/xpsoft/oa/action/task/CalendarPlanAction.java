...
public class CalendarPlanAction extends BaseAction {
	...
	public String list() {
		...
		if (getRequest().getParameter("Q_assignerId_L_EQ") == null) {
			filter.addFilter("Q_userPlan.userId_L_EQ", ContextUtil
	...
	public String get() {
		CalendarPlan calendarPlan = (CalendarPlan) this.calendarPlanService
				.get(this.planId);
		//将数据转成JSON格式
		StringBuffer sb = new StringBuffer("{success:true,data:");
		JSONSerializer serializer=JsonUtil.getJSONSerializer("startTime","endTime");
		sb.append(serializer.exclude(new String[]{"class"}).serialize(calendarPlan));
		sb.append("}");
	...
	public String save() {
		if (this.calendarPlan.getPlanId() == null) {
			...
			for(String i:userIds){
				AppUser appUser=appUserService.get(new Long(i));
				calendarPlan.getUserPlan().add(appUser);
			}
			...
			CalendarPlan cp = (CalendarPlan) this.calendarPlanService
	...
	public String my() {
		...
		for (CalendarPlan plan : planList) {
			...
			sb.append("parent:'0',");
			sb.append("priority:'").append(plan.getUrgent())
					.append("'},");
		}
