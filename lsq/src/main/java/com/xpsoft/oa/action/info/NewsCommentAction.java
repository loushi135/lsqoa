...
public class NewsCommentAction extends BaseAction {
	...
	public String multiDel() {
		...
		if (ids != null) {
			for (String id : ids) {
				NewsComment delComment = (NewsComment) this.newsCommentService
						.get(new Long(id));
				News news = delComment.getNews();
				if (news.getReplyCounts().intValue() > 1) {
					news.setReplyCounts(Integer.valueOf(news
	...
	public String get() {
		NewsComment newsComment = (NewsComment) this.newsCommentService
				.get(this.commentId);
	...
	public String save() {
		News news = (News) this.newsService.get(this.newsComment
				.getNewsId());
		news.setReplyCounts(Integer.valueOf(news.getReplyCounts()
				.intValue() + 1));
		this.newsService.save(news);

		this.newsComment.setAppUser((AppUser) this.appUserService
