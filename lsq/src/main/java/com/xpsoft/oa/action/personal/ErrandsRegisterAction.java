...
public class ErrandsRegisterAction extends BaseAction {
	...
	public String save() {
		...
		if ((ErrandsRegister.FLAG_LEAVE.equals(this.errandsRegister.getFlag())||ErrandsRegister.FLAG_OUT.equals(this.errandsRegister.getFlag())) &&
		(isNew)) {
			...
			if (proDefintion != null) {
				...
				this.processRunService.saveProcessRun(processRun,
						processForm, runInfo);
			} else {
	...
	protected Map<String, ParamField> constructStartFlowMap(
		...
		ParamField pfApprovalName = (ParamField) map
				.get("approvalName");
