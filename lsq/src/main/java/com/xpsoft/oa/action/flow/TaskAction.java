...
import org.jbpm.api.task.Task;
 ...
 public class TaskAction extends BaseAction
 {
   ...
   private com.xpsoft.oa.service.flow.TaskService flowTaskService;
   ...
   public String listForLeader()
   {
	 String userId=getRequest().getParameter("userId");
     PagingBean pb = new PagingBean(0, 99999);
     List tasks = this.flowTaskService.getTaskInfosByUserId(userId, pb);
	...
	private  void removeDuplicate(List list) {
		   for ( int i = 0 ; i < list.size() - 1 ; i ++ ) {
		    for ( int j = list.size() - 1 ; j > i; j -- ) {
		    	TaskInfo taskInfoJ=(com.xpsoft.core.jbpm.pv.TaskInfo)list.get(j);
		    	TaskInfo taskInfoI=(com.xpsoft.core.jbpm.pv.TaskInfo)list.get(i);
		      if (taskInfoJ.getCreateTime().compareTo(taskInfoI.getCreateTime())==0) {
		        list.remove(j);
		      } 
		     } 
		   } 
