package com.xpsoft.oa.action.nameCard;
...
public class NameCardDelAction extends BaseAction {
	...
	public String multiDel() {
		...
		if (ids != null) {
			for (String id : ids) {
				AppRole appRole = (AppRole) this.appRoleService
						.get(new Long(id));
		...
		if (!rights.equals(appRole.getRights())) {
			...
			for (int i = 0; i < funs.length; i++) {
				if (funs[i].startsWith("_")) {
					AppFunction af = this.appFunctionService
							.getByKey(funs[i]);
					if (af != null) {
	...
	public String save() {
		...
		if ((StringUtils.isNotEmpty(isCopy))
			...
			this.appRole = ((AppRole) this.appRoleService
