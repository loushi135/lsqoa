...
public class InMessageAction extends BaseAction {
	...
	public String know() {
		...
		InMessage in = (InMessage) this.inMessageService
				.get(receiveId);
	...
	public String multiRemove() {
		...
		if (ids != null) {
			for (String id : ids) {
				this.inMessage = ((InMessage) this.inMessageService
	...
	public String read() {
		...
		if (userId != null) {
			...
			if (this.inMessage == null) {
				flag = true;
				this.inMessage = this.inMessageService
						.findLatest(userId);
			}
			if (this.inMessage != null) {
				...
				SimpleDateFormat sdf = new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss");
				String date = sdf.format(this.shortMessage
						.getSendTime());
				StringBuffer buff = new StringBuffer(
						"{success:true,message:");
				buff.append("{'receiveId':"
				...
				if (!flag) {
					InMessage in = this.inMessageService
							.findByRead(userId);
					if (in != null)
