 ...
 public class MailFolderAction extends BaseAction
 {
   ...
   public String list()
   {
     ...
     List<MailFolder> mailFolderList = this.mailFolderService.getAllUserFolderByParentId(curUserId, Long.valueOf(0L));
   ...
   public String count()
   {
     ...
     List<MailFolder> mailFolderList = this.mailFolderService.getFolderLikePath(tmpFolder.getPath());
     ...
     if (this.folderId != null) {
       ...
       if ((count != null) && (new Long(count).longValue() > 0L)) {
         ...
         for (Iterator localIterator1 = mailFolderList.iterator(); localIterator1.hasNext(); ) { folder = (MailFolder)localIterator1.next();
           List<MailBox> mailBoxList = this.mailBoxService.findByFolderId(folder.getFolderId());
       ...
       for (MailFolder fd : mailFolderList) {
         this.mailFolderService.remove(fd.getFolderId());
       }
     }
