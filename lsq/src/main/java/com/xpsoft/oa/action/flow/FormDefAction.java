...
import com.xpsoft.oa.model.flow.ExtFormItem;
...
public class FormDefAction extends BaseAction {
	...
	public String list() {
		...
		List nodes = this.jbpmService
				.getFormNodes(new Long(this.defId));

		List<FormDef> formDefs = this.formDefService
	...
	public String get() {
		FormDef formDef = (FormDef) this.formDefService
				.get(this.formDefId);
	...
	public String save() {
		String activityName = getRequest()
		...
		String formPath = AppUtil.getAppAbsolutePath()
		...
		ExtFormItem[] formItems = gson.fromJson("[" + formItemDef
				+ "]", ExtFormItem[].class);
		...
		if (formItems != null) {
			...
			for (ExtFormItem item : formItems) {
				xmlBuf.append("\t<field name=\"" + item.getName()
						+ "\" label=\"" + item.getFieldLabel() +
						"\" type=\"" + item.getType() + "\" length=\""
		...
		if (xmlBuf.length() > 0) {
			String fieldFilePath = formPath + "/" + activityName
					+ "-fields.xml";
	...
	public String getVmXml() {
		...
		setJsonString("{success:true,vmSources:"
