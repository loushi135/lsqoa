...
public class MailAction extends BaseAction {
	...
	public String list() {
		...
		for (MailBox mailBoxTemp : list) {
			...
			buff
					...
					.append("',content:'");
			String content = StringUtil.html2Text(mailTemp
					.getContent());
	...
	public String multiDel() {
		MailFolder deleteFolder = (MailFolder) this.mailFolderService
		...
		if (ids != null) {
			if (getFolderId().longValue() == FOLDER_ID_DELETE) {
				...
				for (String id : ids) {
					MailBox mailBoxDelete = (MailBox) this.mailBoxService
							.get(new Long(id));
					mailBoxDelete.setDelFlag(Short
							.valueOf(HAVE_DELETE));
	...
	public String get() {
		...
		if ((opt == null) || ("".equals(opt))) {
			...
			if (list.size() > 0)
				...
				mailBox = (MailBox) this.mailBoxService
						.get(this.boxId);
			}
		}
		...
		getRequest().setAttribute("mailAttachs",
	...
	public String save() {
		if (this.mail.getMailStatus().shortValue() == IS_MAIL) {
			...
			if ((this.replyBoxId != null)
					&& (!"".equals(this.replyBoxId))) {
				MailBox reply = (MailBox) this.mailBoxService
						.get(this.replyBoxId);
				reply.setReplyFlag(Short.valueOf(HAVE_READ));
				this.mailBoxService.save(reply);
			}

			MailFolder receiveFolder = (MailFolder) this.mailFolderService
					.get(Long.valueOf(FOLDER_ID_RECEIVE));
			MailFolder sendFolder = (MailFolder) this.mailFolderService
			...
			if (this.mail.getMailId() == null) {
				...
				Mail old = (Mail) this.mailService.get(this.mail
						.getMailId());
				try {
					...
					for (String id : fileIds) {
						if (id.equals(""))
							continue;
						mailAttachs
						.add((FileAttach) this.fileAttachService
								.get(new Long(id)));
					}
				...
				MailBox drafted = (MailBox) this.mailBoxService
						.get(this.boxId);
			...
			MailFolder draftFolder = (MailFolder) this.mailFolderService
	...
	public void SaveMail() {
		...
		for (String id : fileIds) {
			if (!id.equals("")) {
				mailAttachs.add((FileAttach) this.fileAttachService
						.get(new Long(id)));
			}
		}
	...
	public String opt() {
		...
		if ((!"".equals(copyToNames)) && (copyToNames != null)) {
			newSubject.append("<br><strong>抄送人</strong>:"
					+ copyToNames);
		}
		newSubject.append("<br><strong>主题</strong>:"
		...
		if (opt.equals("回复")) {
			MailBox replyBox = (MailBox) this.mailBoxService
					.get(this.boxId);
	...
	public String move() {
		...
		if (moveSuccess) {
			for (String id : ids) {
				if (!"".equals(id)) {
					MailBox mailBoxMove = (MailBox) this.mailBoxService
							.get(new Long(id));
					mailBoxMove.setMailFolder(moveToFolder);
					if (this.folderId.longValue() != FOLDER_ID_DELETE)
						mailBoxMove.setDelFlag(Short
								.valueOf(NOT_DELETE));
					else {
						mailBoxMove.setDelFlag(Short
								.valueOf(HAVE_DELETE));
	...
	public String search() {
		...
		String searchContent = getRequest().getParameter(
				"searchContent");
		List<MailBox> list = this.mailBoxService
