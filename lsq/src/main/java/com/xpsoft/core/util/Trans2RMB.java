...
public class Trans2RMB {
	...
	public static void main(String[] args) {
		...
		String s = t2r.cleanZero(t2r.splitNum(t2r.roundString("231.231")));
		// 如果转换过后是一个空串，则不输出屏幕
		if(!"".equals(s)) {
			System.out.println("转换成中文后为：" + s);;
		}
		System.out.println("\n---------------------------------------------");
		System.out.println(t2r.toRMB("-123 "));;
	}
	
	public String toRMB(String arg){
		
		if(arg.trim().startsWith("-")){
			arg=arg.substring(1);
			return "负" +cleanZero(splitNum(roundString(arg)));
		}
		
		return cleanZero(splitNum(roundString(arg)));
	}
