     ...
     if ((orig instanceof DynaBean)) {
       ...
       for (int i = 0; i < origDescriptors.length; i++) {
         ...
         if ((!beanUtils.getPropertyUtils().isReadable(orig, name)) || 
           (!beanUtils.getPropertyUtils().isWriteable(dest, name))) continue;
         try {
 			if(beanUtils.getPropertyUtils().getPropertyType(dest, name) == (beanUtils.getPropertyUtils().getPropertyType(orig, name))){
         ...
         {
        	 throw new IllegalArgumentException("NoSuchMethodException:"+name);
         }
       }
     }
   }
