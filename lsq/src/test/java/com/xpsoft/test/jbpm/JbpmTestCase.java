...
public class JbpmTestCase extends BaseTestCase {
	...
	public void deploy() {
		String id = this.repositoryService.createDeployment()
				.addResourceFromClasspath("com/xpsoft/test/jbpm/test.jpdl.xml")
				.deploy();
		System.out.println("deployId:" + id);
	}

	public void deploySignAll() {
		String id = this.repositoryService
				.createDeployment()
				.addResourceFromClasspath(
						"com/xpsoft/test/jbpm/signAll2.jpdl.xml").deploy();
		System.out.println("deployId:" + id);
	}
	...
	public void getExecutionId() {
		...
		Environment env = environmentFactory.openEnvironment();
		try {
			deployId = task.getProcessInstance()
		...
		ProDefinition pd = this.proDefinitionService
		...
		if (taskEl != null) {
			...
			for (int i = 0; i < trans.size(); i++) {
				...
				System.out.println("transiton name:"
	...
	public void signAll() {
		...
		List<Task> userTaskList = this.taskService
				.findPersonalTasks("3");

		for (Task tk : userTaskList) {
			System.out.println("userid 1:  task:" + tk.getName()
					+ " taskId:" + tk.getId());
		}

		List<Task> userTaskList6 = this.taskService
				.findPersonalTasks("6");

		for (Task tk : userTaskList6) {
			System.out.println("userid 6:  task:" + tk.getName()
					+ " taskId:" + tk.getId());

			TaskImpl tm = (TaskImpl) this.taskService.getTask(tk
					.getId());
			...
			List transitions = this.jbpmService
	...
	public void testGetValue() {
		List list = this.taskDao.getByActivityNameVarKeyLongVal(
